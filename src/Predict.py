...
def getPredictions(frames, predict_batch_size, model_dir, isBGRFormat=False):
    ...
    y_pred = getModel(model_dir).predict(units, batch_size=batch_size, verbose=0)

    y_pred = y_pred.reshape((-1, GRID_ROWS, GRID_COLS, IMGS_PER_INSTANCE, 3))
    y_pred = np.transpose(y_pred, (0, 3, 4, 1, 2))

    confGrid = y_pred[:, :, 0]
    xOffsetGrid = y_pred[:, :, 1]
    yOffsetGrid = y_pred[:, :, 2]

    ballCoordinates = []
    for i in range(confGrid.shape[0]):
        for j in range(confGrid.shape[1]):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Argument Parser for src GridTrackNet Predict")

    parser.add_argument("--video_path", required=True, type=str, help="Path to input .mp4 video file.")
    parser.add_argument("--model_path", required=True, type=str, help="Path to TensorFlow weights file.")
    ...
    try:
        while cap.isOpened():
            ...
            if index % numFramesSkip == 0:
                ...
                if len(frames) == CHUNK_SIZE:
                    ballCoordinates = getPredictions(frames, PREDICT_BATCH_SIZE, MODEL_DIR, True)
                    predictedWindows += len(ballCoordinates) // IMGS_PER_INSTANCE
        ...
        if len(frames) >= IMGS_PER_INSTANCE:
            ...
            if processable_frames:
                ballCoordinates = getPredictions(processable_frames, PREDICT_BATCH_SIZE, MODEL_DIR, True)
                predictedWindows += len(ballCoordinates) // IMGS_PER_INSTANCE
                for ball in ballCoordinates:
                    ballCoordinatesHistory.append(ball)
    ...
    effective_fps = predictedFrameIndex / elapsed if elapsed > 0 else 0.0
    print(f"Model: {MODEL_DIR}")
    print(f"Processed frames: {predictedFrameIndex}")
    print(f"Inference windows: {predictedWindows}")
    print(f"Elapsed seconds: {elapsed:.4f}")
    print(f"FPS: {effective_fps:.4f}")
